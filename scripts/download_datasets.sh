...
if [ -f $PWD/data/ubuntu_corpus_v2/ubuntu_train.pkl ]; then
    ...
    unzip $file_name -d data/ubuntu_corpus_v2
    rm -r $file_name
fi

export file_name=ubuntu_post_training.txt
if [ -f $PWD/data/ubuntu_corpus_v2/$file_name ]; then
    ...
    wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id='$file_id -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=$file_id" -O $file_name && rm -rf /tmp/cookies.txt
    mv $file_name data/ubuntu_corpus_v2/
fi
